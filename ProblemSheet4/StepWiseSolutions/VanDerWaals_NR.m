...
%% solving for molar volume using "van der Waals equation of state" : Newton-Raphson method
% initial guess value 
molar_volume0 = 0.005; % m^3/mol
ea = 100;
...
while(iter <= max_iterations)
    ...
    func = (R*T)/(molar_volume - b) - a/(molar_volume^2) - P;
    func_derivative = -(R*T)/((molar_volume - b)^2) + (2*a)/(molar_volume^3);
...
figure(1);
x = 1:a(2);
y = array1.*100;     % converting relative error to % error

plot(x,y,LineStyle="-",Color='b',Marker='o',MarkerFaceColor=[1, 0.5, 0],LineWidth=1);
hold on;
grid on;
title('"Van Der Waals equation of state" : Newton-Raphson method');
xlabel('Iterations');
